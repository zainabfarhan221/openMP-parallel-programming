...
#define MAX_SIZE 512
#define NUM_RUNS 10
#define CHUNK_SIZE 200// Define the chunk size for dynamic scheduling
...
int main() {
    ...
    for (int t = 0; t < 2; t++) {
        ...
        printf("\n--- Dynamic Scheduling with %d Threads (Chunk Size: %d) ---\n", num_threads[t], CHUNK_SIZE);
        printf("\nMatrix Size: %d x %d\n", N, N);
        ...
        for (int run = 0; run < NUM_RUNS; run++) {
            ...
            determinant = determinant_parallel_dynamic(A, N);
